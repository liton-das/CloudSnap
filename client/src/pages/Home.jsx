...
const handleUpload =async()=>{
  ...
  try {
   let res = await axios.post('http://localhost:4000/img/imgUpload',myFormData,{
    ...
    getMessage.error('somthing went worng!')
    setLoading(false)
    console.log(error)
  }
  finally{
      ...
      <div className="min-h-screen bg-slate-50 p-6 flex flex-col items-center">
        <div className="w-full max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="col-span-1 md:col-span-2">
                <div
                  ...
                  className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center h-56 md:h-72"
                >
                  {Img ? (
                    <>
                      <div className="w-[600px] relative flex justify-center items-center rounded-[5px] h-[400px] overflow-hidden">
                        <img className="w-full" src={Img} alt="Image" />
                      </div>
                      <button
                        onClick={handleRemoveImg}
                        className="mt-3 absolute top-[35%] inline-flex items-center px-4 py-2 rounded-lg bg-red-500 opacity-[0.9] cursor-pointer text-white text-sm shadow-sm"
                      >
                        Remove
                      </button>
                    </>
                ...
                <div className="mt-4 text-sm text-slate-500">
                  <ul className="list-disc pl-5 space-y-1">
                    <li>
                      Image Link :
                      <a className="text-blue-500" target="_blank" href={uploadedImg[0]?.imageLink}>
                        ({uploadedImg[0]?.imageLink}).
                      </a>
                    </li>
                  </ul>
                </div>
          ...
          <div className="mt-6 text-xs text-slate-400">
            <p>
              Note: This component is presentational only — integrate file input, drag/drop, state
              and upload handlers where needed.
            </p>
          </div>
        </div>
      </div>
